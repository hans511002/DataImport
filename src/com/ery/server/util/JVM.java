...
/**
 * This class is a wrapper for the implementation of com.sun.management.UnixOperatingSystemMXBean It will decide to use the sun api or its
 * own implementation depending on the runtime (vendor) used.
 */
...
public class JVM {
	...
	/**
	 * @return the physical free memory (not the JVM one, as it's not very useful as it depends on the GC), but the one from the OS as it
	 *         allows a little bit more to guess if the machine is overloaded or not).
	 */
	...
	/**
	 * Get the number of the maximum file descriptors the system can use. If Oracle java, it will use the com.sun.management interfaces.
	 * Otherwise, this methods implements it (linux only).
	 * 
