...
public class Bytes {
	...
	/**
	 * A {@link ByteArrayComparator} that treats the empty array as the largest
	 * value. This is useful for comparing row end keys for regions.
	 */
	// TODO: unfortunately, HBase uses byte[0] as both start and end keys for
	// region
	// boundaries. Thus semantically, we should treat empty byte array as the
	// smallest value
	// while comparing row keys, start keys etc; but as the largest value for
	// comparing
	...
	/**
	 * Copy the byte array given in parameter and return an instance of a new
	 * byte array with the same length and the same content.
	 * 
	 * @param bytes
	 ...
	 * or copied it from somewhere. (mcorgan)
	 * 
	 * @param a
	 ...
	 * @return The index of key if found. If not found, return -(index + 1),
	 *         where negative indicates "not found" and the "index + 1" handles
	 *         the "-0" case.
	 */
	...
	/**
	 * Treat the byte[] as an unsigned series of bytes, most significant bits
	 * first. Start by adding 1 to the rightmost bit/byte and carry over all
	 * overflows to the more significant bits/bytes.
	 * 
	 * @param input
	 *            The byte[] to increment.
	 * @return The incremented copy of "in". May be same length or 1 byte
	 *         longer.
	 */
	 ...
	 * @param target
	 *            a primitive {@code byte} value
	 * @return {@code true} if {@code target} is present as an element anywhere
	 *         in {@code array}.
	 */
	 ...
	 * @return {@code true} if {@code target} is present anywhere in
	 *         {@code array}
	 */
	public static boolean contains(byte[] array, byte[] target) {
		return indexOf(array, target) > -1;
