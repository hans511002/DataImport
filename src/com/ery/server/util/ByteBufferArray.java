...
/**
 * This class manages an array of ByteBuffers with a default size 4MB. These buffers are sequential and could be considered as a large
 * buffer.It supports reading/writing data from this large buffer with a position and offset
 */
public final class ByteBufferArray {
	...
	public ByteBufferArray(long capacity, boolean directByteBuffer) {
		...
		LOG.info("Allocating buffers total=" + StringUtils.byteDesc(capacity) + " , sizePerBuffer=" + StringUtils.byteDesc(bufferSize)
				+ ", count=" + bufferCount);
	 ...
	 * 
	 * @param start
	 *            start position in the ByteBufferArray
	 * @param len
	 ...
	 *            The array into which bytes are to be written
	 * @param dstOffset
	 *            The offset within the given array of the first byte to be written
	 */
	...
	private interface Visitor {
		/**
		 * Visit the given byte buffer, if it is a read action, we will transfer the bytes from the buffer to the destination array, else if
		 * it is a write action, we will transfer the bytes from the source array to the buffer
		 * 
		 * @param bb
		 *            byte buffer
		 * @param array
		 *            a source or destination byte array
		 * @param arrayOffset
	...
	void multiple(long start, int len, byte[] array, int arrayOffset, Visitor visitor) {
		...
		if (startBuffer >= locks.length || startBuffer < 0) {
			String msg = "Failed multiple, start=" + start + ",startBuffer=" + startBuffer + ",bufferSize=" + bufferSize;
			LOG.error(msg);
