...
public final class ByteBufferUtils {
	...
	/**
	 * Put in buffer integer using 7 bit encoding. For each written byte: 7 bits
	 * are used to store value 1 bit is used to indicate whether there is next
	 * bit.
	 * 
	 * @param value
	 ...
	 *            the buffer to read bytes from
	 * @param offset
	 *            the offset in the buffer (from the buffer's array offset) to
	 *            start copying bytes from
	 * @param length
	 *            the number of bytes to copy
	 */
	...
	public static int putLong(OutputStream out, final long value, final int fitInBytes) throws IOException {
		long tmpValue = value;
	...
	/**
	 * Copy from the InputStream to a new heap ByteBuffer until the InputStream
	 * is exhausted.
	 */
	 ...
	 */
	public static int findCommonPrefix(byte[] left, int leftOffset, int leftLength, byte[] right, int rightOffset,
			int rightLength) {
	 ...
	 */
	public static boolean arePartsEqual(ByteBuffer buffer, int offsetLeft, int lengthLeft, int offsetRight,
			int lengthRight) {
		...
		if (buffer.hasArray()) {
			return 0 == Bytes.compareTo(buffer.array(), buffer.arrayOffset() + offsetLeft, lengthLeft, buffer.array(),
					buffer.arrayOffset() + offsetRight, lengthRight);
		}
	...
	/**
	 * Copy the bytes from position to limit into a new byte[] of the exact
	 * length and sets the position and limit back to their original values
	 * (though not thread safe).
	 * 
	 * @param buffer
	 *            copy from here
	 * @param startPosition
	 *            put buffer.get(startPosition) into byte[0]
