...
import com.ery.server.conf.Configured;
import com.ery.server.conf.ImportConfiguration;
...
public abstract class ServerCommandLine extends Configured implements Tool {
	 ...
	 * {@code "hbase.envvars.logging.disabled"} to {@code "true"}.
	 * <p>
	 * If enabled, you can also exclude environment variables containing certain
	 * substrings by setting {@code "hbase.envvars.logging.skipwords"} to comma
	...
	/**
	 * Parse and run the given command line. This may exit the JVM if a nonzero
	 * exit code is returned from <code>run()</code>.
	 */
	public void doMain(String args[]) {
		try {
			int ret = ToolRunner.run(ImportConfiguration.create(), this, args);
			if (ret != 0) {
				System.exit(ret);
			}
		} catch (Exception e) {
