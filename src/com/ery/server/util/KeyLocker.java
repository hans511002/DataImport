...
public class KeyLocker<K extends Comparable<? super K>> {
	...
	// it's equal to zero.
	private final Map<K, Pair<KeyLock<K>, AtomicInteger>> locks = new HashMap<K, Pair<KeyLock<K>, AtomicInteger>>(NB_CONCURRENT_LOCKS);
	...
	private synchronized void releaseLock(K key) {
		...
		if (lock != null) {
			...
			String message = "Can't release the lock for " + key + ", this key is not in the key list." + " known keys are: "
					+ locks.keySet();
			LOG.error(message);
