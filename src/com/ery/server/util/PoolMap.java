...
public class PoolMap<K, V> implements Map<K, V> {
	 ...
	 * builds on the {@link LinkedList} class. It essentially allows resources
	 * to be checked out, at which point it is removed from this pool. When the
	 * resource is no longer required, it should be returned to the pool in
	 * order to be reused.
	 * 
	 * <p>
	 * If {@link #maxSize} is set to {@link Integer#MAX_VALUE}, then the size of
	 * the pool is unbounded. Otherwise, it caps the number of consumers that
	 * can check out a resource from this pool to the (non-zero positive) value
	 ...
	 * builds on the {@link ThreadLocal} class. It essentially binds the
	 * resource to the thread from which it is accessed.
	 * 
	 * <p>
	 * Note that the size of the pool is essentially bounded by the number of
	 * threads that add resources to this pool.
	 * </p>
	 * 
	 * @param <R>
	 *            the type of the resource
