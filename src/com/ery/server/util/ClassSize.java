...
public class ClassSize {
	...
	static {
		...
		BYTE_BUFFER = align(OBJECT + align(REFERENCE) + align(ARRAY) + (5 * Bytes.SIZEOF_INT) + (3 * Bytes.SIZEOF_BOOLEAN)
				+ Bytes.SIZEOF_LONG);

		INTEGER = align(OBJECT + Bytes.SIZEOF_INT);
		LONG = align(OBJECT + Bytes.SIZEOF_INT);
		DOUBLE = align(OBJECT + Bytes.SIZEOF_DOUBLE);
		DATE = align(OBJECT + Bytes.SIZEOF_LONG);
	...
	private static long estimateBaseFromCoefficients(int[] coeff, boolean debug) {
		...
		if (debug) {
			if (LOG.isDebugEnabled()) {
				LOG.debug("Primitives=" + coeff[0] + ", arrays=" + coeff[1] + ", references(includes " + nrOfRefsPerObj
						+ " for object overhead)=" + coeff[2] + ", refSize " + REFERENCE + ", size=" + size + ", prealign_size="
						+ prealign_size);
			}
		}
		return size;
