...
public final class WritableUtils {
	...
	public static void write(DataOutput out, byte[] s) throws IOException {
		if (s != null) {
			writeVInt(out, s.length);
			if (s.length > 0)
				out.write(s);
		}
	}
	...
	/*
	 * Write a String array as a Nework Int N, followed by Int N Byte Array of compressed Strings. Handles also null arrays and null values.
	...
	/*
	 * Write a String array as a Nework Int N, followed by Int N Byte Array Strings. Could be generalised using introspection. Actually this
	 * bit couldn't...
	 */
	...
	/*
	 * Write a String array as a Nework Int N, followed by Int N Byte Array Strings. Could be generalised using introspection. Handles null
	...
	public static <T extends Serializable> T clone(T orig) {
		try {
			T newInst = null;
			ReflectionUtils.copy(orig, newInst);
			return newInst;
		} catch (IOException e) {
	...
	/**
	 * Serializes an integer to a binary stream with zero-compressed encoding. For -120 <= i <= 127, only one byte is used with the actual
	 * value. For other values of i, the first byte value indicates whether the integer is positive or negative, and the number of bytes
	 * that follow. If the first byte value v is between -121 and -124, the following integer is positive, with number of bytes that follow
	 * are -(v+120). If the first byte value v is between -125 and -128, the following integer is negative, with number of bytes that follow
	 * are -(v+124). Bytes are stored in the high-non-zero-byte-first order.
	 * 
	 * @param stream
	...
	/**
	 * Serializes a long to a binary stream with zero-compressed encoding. For -112 <= i <= 127, only one byte is used with the actual
	 * value. For other values of i, the first byte value indicates whether the long is positive or negative, and the number of bytes that
	 * follow. If the first byte value v is between -113 and -120, the following long is positive, with number of bytes that follow are
	 * -(v+112). If the first byte value v is between -121 and -128, the following long is negative, with number of bytes that follow are
	 * -(v+120). Bytes are stored in the high-non-zero-byte-first order.
	 * 
	 * @param stream
	 *            Binary output stream
	 * @param i
	...
	public static <T extends Enum<T>> T readEnum(DataInput in, Class<T> enumType) throws IOException {
		return T.valueOf(enumType, readString(in));
	}
