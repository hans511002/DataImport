...
public class HttpServer implements FilterContainer {
	...
	static final String FILTER_INITIALIZER_PROPERTY = "http.filter.initializers";

	// The ServletContext attribute where the daemon Configuration
	// gets stored.
	public static final String CONF_CONTEXT_ATTRIBUTE = "server.conf";
	...
	public Map<String, BaseServlet> servletMap = new HashMap<String, BaseServlet>();
	...
	public HttpServer(String name, String bindAddress, int port, boolean findPort, Configuration conf,
		...
		// try {
		// if (servletMap.containsKey(s)) {
		// servletMap.get(s).doGet(request, response);
		// } else {
		// str = "未找到" + s + "页面!";
		// }
		// } catch (Exception e) {
		...
		// webAppContext.setWelcomeFiles(new String[] { "master" });
	...
	private static FilterInitializer[] getFilterInitializers(Configuration conf) {
		...
		for (int i = 0; i < classes.length; i++) {
			initializers[i] = (FilterInitializer) Configuration.newInstance(classes[i], conf);
		}
		return initializers;
	}
	...
	protected void addDefaultApps(ContextHandlerCollection parent, final String appDir) throws IOException {
		...
		// defined.
		String logDir = System.getProperty("dimport.log.dir");
		if (logDir != null) {
	 ...
	 * protect with Kerberos authentication. Note: This method is to be used for
	 ...
	 * (except internal Kerberos filters) are not enabled.
	 * 
	 * @param name
	 *            The name of the servlet (can be passed as null)
	 * @param pathSpec
	 ...
	 * @param useKsslForAuth
	 *            true to use KSSL for auth, false to use SPNEGO
	 */
	public void addInternalServlet(String name, String pathSpec, Class<? extends HttpServlet> clazz,
			boolean requireAuth, boolean useKsslForAuth) {
			...
			if (listenerStartedExternally) { // Expect that listener was started
				...
				while (true) {
					try {
						...
						LOG.info("Port returned by webServer.getConnectors()[0]." + "getLocalPort() before open() is " +
						...
						// Workaround to handle the problem reported in
						// HADOOP-4744
						if (port < 0) {
							Thread.sleep(100);
							int numRetries = 1;
	 ...
	 * If it isn't the case, response will be modified to send an error to the
	 * user.
	 * 
	...
	public static class QuotingInputFilter implements Filter {
		...
		public static class RequestQuoter extends HttpServletRequestWrapper {
			...
			/**
			 * Quote the server name so that users specifying the HOST HTTP
			 * header can't inject attacks.
			 */
			@Override
