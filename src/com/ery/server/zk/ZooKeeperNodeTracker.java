...
public abstract class ZooKeeperNodeTracker extends ZooKeeperListener {
	 ...
	 * @param abortable
	 */
	public ZooKeeperNodeTracker(ZooKeeperWatcher watcher, String node, Server abortable) {
		super(watcher);
		this.node = node;
	 ...
	 * or {@link #getData(boolean)} to get the data of the node if it is
	 * available.
	 */
		...
		try {
			if (ZKUtil.watchAndCheckExists(watcher, node)) {
				...
				if (data != null) {
					...
					// It existed but now does not, try again to ensure a watch
					// is set
		...
		while (!this.stopped && (notimeout || remaining > 0) && this.data == null) {
			if (!nodeExistsChecked) {
				try {
					...
					LOG.warn("Got exception while trying to check existence in  ZooKeeper" + " of the node: " + node +
							", retrying if timeout not reached", e);
				} catch (IOException e) {
					LOG.warn("Got exception while trying to check existence in  ZooKeeper" + " of the node: " + node +
							", retrying if timeout not reached", e);
				}
				...
				if (nodeExistsChecked) {
					...
					try {
						// This does not create a watch if the node does not
						// exists
						this.data = ZKUtil.getDataAndWatch(watcher, node);
					} catch (KeeperException e) {
						LOG.warn("Unexpected exception handling blockUntilAvailable", e);
	...
	public boolean checkIfBaseNodeAvailable() {
		try {
			...
			abortable.abort("Exception while checking if basenode (" + watcher.baseZNode + ") exists in ZooKeeper.", e);
		} catch (IOException e) {
			abortable.abort("Exception while checking if basenode (" + watcher.baseZNode + ") exists in ZooKeeper.", e);
		}
		return true;
