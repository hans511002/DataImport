 ...
 * 
 * The {@link ZooKeeperWatcher} for a process will execute the appropriate methods of implementations of this class. In order to receive
 * events from the watcher, every listener must register itself via {@link ZooKeeperWatcher#registerListener}.
 * 
 * Subclasses need only override those methods in which they are interested.
 * 
 * Note that the watcher will be blocked when invoking methods in listeners so they must not be long-running.
 */
