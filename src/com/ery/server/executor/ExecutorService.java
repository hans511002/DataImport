...
import com.ery.server.executor.EventHandler.EventHandlerListener;
import com.ery.server.monitor.ThreadMonitoring;

/**
 * This is a generic executor service. This component abstracts a threadpool, a
 * queue to which {@link EventType}s can be submitted, and a
 ...
 * {@link #registerListener(EventType, EventHandler.EventHandlerListener)}. Be
 * sure to deregister your listener when done via
 * {@link #unregisterListener(EventType)}.
 */
public class ExecutorService {
	 ...
	 * {@link EventType}. Currently only one listener per event type.
	 * 
	 * @param type
	 ...
	 *            The listener to run.
	 */
	public void registerListener(final EventType type, final EventHandlerListener listener) {
	...
	static class Executor {
		...
		protected Executor(String name, int maxThreads, final Map<EventType, EventHandlerListener> eventHandlerListeners) {
			...
			this.threadPoolExecutor = new TrackingThreadPoolExecutor(maxThreads, maxThreads, keepAliveTimeInMillis,
