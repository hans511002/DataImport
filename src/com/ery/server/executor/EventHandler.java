 ...
 */
package com.ery.server.executor;
 ...
 * the {@link #process()} and {@link #prepare()} methods. Subclasses should also
 * do all necessary checks up in their prepare() if possible -- check table
 * exists, is disabled, etc. -- so they fail fast rather than later when process
 * is running. Do it this way because process be invoked directly but event
 * handlers are also run in an executor context -- i.e. asynchronously -- and in
 * this case, exceptions thrown at process time will not be seen by the invoker,
 * not till we implement a call-back mechanism so the client can pick them up
 * later.
 * <p>
 ...
 * executor, see ExecutorService, has a switch for passing event type to
 * executor.
 * <p>
...
public abstract class EventHandler implements Runnable, Comparable<Runnable> {
	...
	/**
	 * Event handlers should do all the necessary checks in this method (rather
	 * than in the constructor, or in process()) so that the caller, which is
	 * mostly executed in the ipc context can fail fast. Process is executed
	 * async from the client ipc, so this method gives a quick chance to do some
	 * basic checks. Should be called after constructing the EventHandler, and
	 * before process().
	 * 
